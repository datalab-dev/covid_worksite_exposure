...
**Historical Data Scraping:** The Internet Archive (also known as The Wayback Machine), as it's name suggests, archives websites making previous versions of the webpages available at a later date, including the UC Davis Potential Worksite Exposures Reporting page. Since the current website only displays data for 14 days, we scraped the Internet Archive to obtain the data that appeared on the site before we started this project, which included the exposures data reported from January to August 2021. The data obtained from the Internet Archive was added to the dataset we have amassed in scraping the current website daily. The process of scraping the Internet Archive was similar to the one we use to scrape the current site but required some preparation steps that would allow scraping specific to the Internet Archive. We used the Wayback Machine Scraper (https://github.com/sangaline/wayback-machine-scraper) to retrieve a list of the archived URLs available on the site. Once we had the list of URLs, scraping the data followed a very similar workflow to our daily scraping script.
