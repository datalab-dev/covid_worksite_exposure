...
**Historical Data Scraping:** The Internet Archive WayBack Machine is in charge of scraping some sites html in order to keep a record of previous versions of the webpage. Since the campusready.ucdavis.edu website only holds a static number of exposures at a time, the WayBack Machine was scraped to obtain all of the called 'Historical Data', refering to all the exposures data reported from January to August. This process was only required once and added to the dataset from the 'Scrape Data' section mentioned above. A similar process to that of said section was also used, starting with some preparation steps that would allow scraping specific to the WayBack Machine. The Wayback Machine Scraper (https://github.com/sangaline/wayback-machine-scraper), allowed the retrieval of the URLs, which were modified by eliminating timestamps that were page specific, and then imputting the page number within the code to obtain exposures from multiple pages.  
